...
public class Events implements Listener {
	...
	public void onTeleport(PlayerTeleportEvent e) {
		...
		} else if(Cache.gamemodes.containsKey(e.getPlayer().getName())){ e.getPlayer().setGamemode(Cache.gamemodes.get(e.getPlayer().getName())); Cache.gamemodes.remove(e.getPlayer().getName()); }
	...
	public void onForm(PlayerFormRespondedEvent e) {
		...
		if(e.getWindow() instanceof FormWindowCustom) {
			...
			if(fw.getTitle().startsWith("§3WorldSettings")) {
				...
				if(c.getBoolean("UseOwnGamemode")) for(Player p : Server.getInstance().getLevelByName(level).getPlayers().values()) if(p.getGamemode() != c.getInt("Gamemode")) {Cache.gamemodes.put(p.getName(), (byte) p.getGamemode()); p.setGamemode(c.getInt("Gamemode"));}
				...
				try {
					...
					Config c = new Config(new File(Server.getInstance().getDataPath() + "worlds/" + level.getName(), "config.yml"));
					for(int i = 0; i < levels.size() ; i++) {
					
						if(fw.getResponse().getToggleResponse(i)) {
							
							Config c2 = new Config(new File(Server.getInstance().getDataPath() + "worlds/" + levels.get(i).getName(), "config.yml"));
							c2.setAll((LinkedHashMap<String, Object>) c.getAll());
							c2.save();
							levels.get(i).gameRules = level.gameRules;
							Cache.initWorld(levels.get(i));
						}
